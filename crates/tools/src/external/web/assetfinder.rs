...
#[async_trait]
impl PentestTool for AssetfinderTool {
    fn name(&self) -> &str {
        "assetfinder"
    }
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        execute_timed(|| async move {
            ...
            ensure_tool_installed(&platform, "assetfinder", "assetfinder").await?;
            ...
            let result = platform
                .execute_command("assetfinder", &args_refs, Duration::from_secs(timeout_secs))
                .await?;

            let domains: Vec<String> = result.stdout.lines().map(|s| s.to_string()).collect();
            Ok(json!({
                "domain": domain,
