//! Subfinder - Subdomain discovery tool using passive sources
...
#[async_trait]
impl PentestTool for SubfinderTool {
    fn name(&self) -> &str {
        "subfinder"
    }
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        execute_timed(|| async move {
            ...
            ensure_tool_installed(&platform, "subfinder", "subfinder").await?;
            ...
            let builder = CommandBuilder::new().arg("-d", &domain).flag("-silent");

            let args = builder.build();
            let args_refs: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
            let result = platform
                .execute_command("subfinder", &args_refs, Duration::from_secs(timeout_secs))
                .await?;

            let subdomains: Vec<String> = result.stdout.lines().map(|s| s.to_string()).collect();
            Ok(json!({
                "domain": domain,
