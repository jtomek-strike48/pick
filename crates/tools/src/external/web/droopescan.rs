...
pub struct DroopescanTool;

#[async_trait]
impl PentestTool for DroopescanTool {
    fn name(&self) -> &str {
        "droopescan"
    }

    fn description(&self) -> &str {
        "Scanner for Drupal, Joomla, Moodle, SilverStripe, and WordPress"
    }
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        execute_timed(|| async move {
            ...
            ensure_tool_installed(&platform, "droopescan", "droopescan").await?;
            ...
            let result = platform
                .execute_command("droopescan", &args_refs, Duration::from_secs(timeout_secs))
                .await?;
