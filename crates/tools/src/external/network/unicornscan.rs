...
pub struct UnicornscanTool;

#[async_trait]
impl PentestTool for UnicornscanTool {
    fn name(&self) -> &str {
        "unicornscan"
    }

    fn description(&self) -> &str {
        "Asynchronous network stimulus delivery/response recording tool"
    }
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        execute_timed(|| async move {
            ...
            ensure_tool_installed(&platform, "unicornscan", "unicornscan").await?;
            ...
            let result = platform
                .execute_command("unicornscan", &args_refs, Duration::from_secs(timeout_secs))
                .await?;
