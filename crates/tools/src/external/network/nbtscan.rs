...
pub struct NbtscanTool;

#[async_trait]
impl PentestTool for NbtscanTool {
    fn name(&self) -> &str {
        "nbtscan"
    }
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        execute_timed(|| async move {
            ...
            ensure_tool_installed(&platform, "nbtscan", "nbtscan").await?;
            ...
            let result = platform
                .execute_command("nbtscan", &args_refs, Duration::from_secs(timeout_secs))
                .await?;

            let mut netbios_names = Vec::new();
            for line in result.stdout.lines() {
                if !line.is_empty() && !line.starts_with("Doing") {
                    netbios_names.push(line.trim().to_string());
                }
            }
