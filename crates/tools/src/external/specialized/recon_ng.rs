...
pub struct ReconNgTool;

#[async_trait]
impl PentestTool for ReconNgTool {
    fn name(&self) -> &str {
        "recon_ng"
    }

    fn description(&self) -> &str {
        "Full-featured reconnaissance framework with modular design"
    }
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        execute_timed(|| async move {
            ...
            ensure_tool_installed(&platform, "recon-ng", "recon-ng").await?;
            ...
            let result = platform
                .execute_command("recon-ng", &args_refs, Duration::from_secs(timeout_secs))
                .await?;
