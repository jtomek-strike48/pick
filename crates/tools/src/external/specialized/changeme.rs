...
pub struct ChangemeTool;

#[async_trait]
impl PentestTool for ChangemeTool {
    fn name(&self) -> &str {
        "changeme"
    }

    fn description(&self) -> &str {
        "Scanner for default and weak credentials"
    }
    ...
    async fn execute(&self, params: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        execute_timed(|| async move {
            ...
            ensure_tool_installed(&platform, "changeme", "changeme").await?;
            ...
            let result = platform
                .execute_command("changeme", &args_refs, Duration::from_secs(timeout_secs))
                .await?;

            let mut creds = Vec::new();
            for line in result.stdout.lines() {
                if line.contains("FOUND") || line.contains("SUCCESS") {
                    creds.push(line.to_string());
                }
            }
